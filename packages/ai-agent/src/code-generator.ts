...
import type { ProjectArchitecture, GeneratedCode, ProjectInput, FileDefinition, IntegrationCode } from "./types.js";
...
export function createBatches(architecture: ProjectArchitecture): Array<{
  ...
  while (pages.length > 0 || components.length > 0 || routes.length > 0) {
    ...
    const parts: string[] = [];
    if (batch.pages.length) parts.push(`${batch.pages.length} pages`);
    if (batch.components.length) parts.push(`${batch.components.length} components`);
    if (batch.routes.length) parts.push(`${batch.routes.length} API routes`);
...
async function generateChunked(
  ...
  const allIntegrationCode: IntegrationCode[] = [];
  
  // Track previously generated files for context
  const previousFiles: Array<{ path: string; description: string }> = [];
